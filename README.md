...
Clone 前先在 Docker 沙箱裡掃描，防止惡意 repo 攻擊你的主機。
...
- 惡意套件可能藏在 `package.json` 的 `postinstall` 裡
- Secrets 可能已經洩漏在 commit history 中
- 依賴套件可能有已知漏洞

**Safe Clone** 在 Docker 隔離環境中幫你完成所有檢查，你的主機完全不會被碰到。
...
- **GitHub API 預檢** — clone 前先評估 repo 風險（stars、年齡、owner 等）
